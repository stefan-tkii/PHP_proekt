<?php
    ...
    $found_user = mysqli_fetch_assoc($result_query);

    if($found_user)
    {
        if($user['username'] == $uname) {array_push($errors, 'Username is already taken.');}
        if($user['email'] == $email) {array_push($errors, 'Email is already taken.');}
    }

    if(count($errors)==0)
    {
        $hashed_pass = crypt($pass1);
        ...
        $_SESSION['succsess_register'] = "You have succsessfully registered!";
